...
def food_moment_id_fetch(food_moment: str) -> dict:
    ...
    if not food_moment_id:
        id_from_ai = food_moment_id_from_ai(food_moment,
            food_moments["resultset"])
        if self_debug:
...
def create_dish_func(params: Any = None) -> str:
    """
    Create a new user dish with a list of ingredients and quantities and no date.
    """
    ...
    for ingredient in params.ingredients:
        new_ingredient = {
            "id": str(uuid4()) if None or not isinstance(ingredient.id, str) \
                else ingredient.id,
...
def create_daily_meal_func(params: Any) -> str:
    ...
    for ingredient in params.ingredients:
        ...
        new_ingredient = {
            "id": str(uuid4()) if None or not isinstance(ingredient.id, str) else ingredient.id,
...
def get_daily_meal_list_raw() -> dict:
    ...
    # for row in resultset['resultset']:
    #     for item in row.get("ingredients", []):
    #         if 'meal_type' in item:
    #             continue
    #         item['meal_type'] = "N/A"
    #         if not 'food_moment_id' in item:
    #             continue
    #         if '$oid' in item['food_moment_id']:
    #             if isinstance(item['food_moment_id'], str):
    #                 item['food_moment_id'] = json.loads(item['food_moment_id'])
    #             item['food_moment_id'] = str(item['food_moment_id']['$oid'])
    #         item['meal_type'] = food_moments.get(item['food_moment_id'], "N/A")

    # if DEBUG:
    #     log_debug('AI_GDMLR-2) get_daily_meal_list_raw |' +
    #               f' resultset: {resultset}')
    return resultset
...
def get_user_profile_raw() -> dict:
    ...
    user_data["minimun_daily_calories"] = fu_lib.get_minimum_daily_calories(
        weight=fu_lib.convert_weight(user_data.get("weight", 0),
            user_data.get("weight_unit", "kg"), "kg"),
        height=fu_lib.convert_height(user_data.get("height", 0),
